...
function processUploadedFiles(files) {
  Array.from(files).forEach(file => {
    ...
    // Handle PDF File local extraction
    if (isPDF) {
      ...
      reader.onload = async (e) => {
        try {
          ...
          progressItem.querySelector('.status').innerText = `Parsing pages (0/${pdf.numPages})...`;
          
          for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
            progressItem.querySelector('.status').innerText = `Parsing page ${pageNum}/${pdf.numPages}...`;
            const page = await pdf.getPage(pageNum);
            const textContent = await page.getTextContent();
            const pageText = textContent.items.map(item => item.str).join(" ");
            fullText += pageText + "\n\n";
          ...
          if (cleanText.length === 0) {
            throw new Error("Empty PDF (could be a scanned image).");
          }
          ...
          console.error(err);
          progressItem.querySelector('.status').innerHTML = `<span style="color:var(--danger-color)">${err.message}</span>`;
        }
      };
